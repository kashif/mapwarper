...
function update_gcp(gcp_id, listele) {
  ...
  for (i=0;i<listele.childNodes.length; i++){
    ...
    for (e=0;e<listtd.childNodes.length; e++){
      ...
      if (listItem.id == "x") x = listItem.value;
      if (listItem.id == "y") y = listItem.value;
      if (listItem.id == "lon") lon = listItem.value;
      if (listItem.id == "lat") lat = listItem.value;
...
function move_map_markers(gcp_id, elem){
  ...
  for (i=0;i<trele.childNodes.length; i++){
    ...
    for (e=0;e<trchild.childNodes.length; e++){
      ...
      if (inp.id == 'x') x = inp.value;
      if (inp.id == 'y') y = image_height - inp.value ;
      if (inp.id == 'lon') tlon = inp.value;
      if (inp.id == 'lat') tlat = inp.value;
    }
  }
  ...
  if (attrib == 'x' || attrib == 'y') {
    ...
    if (attrib == 'x') x = avalue;
    if (attrib == 'y') y = image_height - avalue;
    ...
    if (attrib == 'lon') tlon = avalue;
    if (attrib == 'lat') tlat = avalue;
  ...
  for (i=0;i<listele.childNodes.length; i++){
    ...
    for (e=0;e<listtd.childNodes.length; e++){
      ...
      if (feature.layer == from_vectors){
        if (listItem.id == "x") listItem.value = feature.geometry.x;
        if (listItem.id == "y") listItem.value = image_height - feature.geometry.y;
      }
      if (feature.layer == to_vectors){
        ...
        if (listItem.id == "lon") listItem.value = vll.lon;
        if (listItem.id == "lat") listItem.value = vll.lat;
      }
    }//for 
  }//for
...
//    mds = new OpenLayers.Control.MouseDefaults();
    ...
    if (map_has_bounds) {
        ...
        to_map.setCenter(lonLatToMercator(new OpenLayers.LonLat(-81.9, 37.4)), 5);
...
function newaddGCPto(feat) {
  if (active_to_vectors.features.length > 1) {
    ...
    for (var a=0; a< active_to_vectors.features.length; a++) {
      if (active_to_vectors.features[a] != feat)  to_destroy.push(active_to_vectors.features[a]);
    }
...
function newaddGCPfrom(feat) {
  if (active_from_vectors.features.length > 1) {
    ...
    for (var a=0; a< active_from_vectors.features.length; a++) {
      if (active_from_vectors.features[a] != feat)  to_destroy.push(active_from_vectors.features[a]);
    }
